...
function EntityCard({ entity }: { entity: EntityResult }) {
  ...
  return (
    <button
      style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 6, padding: '12px 14px', borderRadius: 6, border: 'none', backgroundColor: '#e5e7eb', width: 160, flexShrink: 0, textAlign: 'left', cursor: 'pointer', transition: 'background-color 0.1s' }}
    ...
    : (value ?? '')
        ...
        .map(part => {
          ...
          if (terms.has(part.toLowerCase())) {
            return `<mark style="background:#fef08a;color:inherit;border-radius:2px;padding:0 1px;">${escapeHtml(part)}</mark>`;
          }
...
function highlightText(text: string, query: string): string {
  ...
  const result = text.replace(regex, '<mark style="background:#fef08a;color:inherit;border-radius:2px;padding:0 1px;">$1</mark>');
  return result;
}
...
function PreviewPanel({ result }: { result: SearchEvidenceResult }) {
  ...
  return (
    <div style={{ width: 310, maxWidth: 310, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
      {/* Two-part card */}
      <div style={{ border: '1px solid var(--border)', borderRadius: 12, backgroundColor: '#f3f4f6', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flexShrink: 0, padding: '10px 14px', display: 'flex', flexWrap: 'wrap', gap: 5 }}>
          {([
            ...
            <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 500, padding: '4px 10px', borderRadius: 99, backgroundColor: '#e5e7eb', color: '#374151', whiteSpace: 'nowrap' }}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col overflow-hidden" style={{ backgroundColor: 'var(--base)' }}>
      ...
      >
        ...
        <button
          style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, border: '1px solid var(--border)', color: 'var(--text-weak)', backgroundColor: 'transparent', lineHeight: '18px' }}
        >
          About Search
        </button>
      </div>
      ...
      {!hasResults ? (
          ...
          <div style={{ maxWidth: '70%', margin: '0 auto' }}>
            ...
            {!isLoading && (
              <div style={{ marginTop: 48 }}>
                <p style={{ fontSize: 12, fontWeight: 500, color: '#9ca3af', marginBottom: 8 }}>Recent searches</p>
        ...
        /* ── Results state ── */
        <div className="flex-1 overflow-hidden flex flex-col" style={{ paddingTop: 48, maxWidth: '70%', margin: '0 auto', width: '100%' }}>
          {/* Search input */}
            ...
            <div className="flex-1 overflow-hidden flex flex-col min-w-0">
              ...
              {isLoading
                ...
                : searchOutput.entities.length > 0 && (
                  <div style={{ marginBottom: 24, paddingTop: 8 }}>
                    <p style={{ fontSize: 12, fontWeight: 500, color: '#9ca3af', marginBottom: 8 }}>Other Results</p>
